// --- src/app/lesson/[lessonId]/page.js (THE DEFINITIVE FINAL MASTERPIECE) ---
'use client';

import React, { useState, useEffect } from 'react';
import { useParams } from 'next/navigation';
import { useChatbot } from '../../providers';
...
export default function LessonPage() {
    ...
    const { setBotId } = useChatbot(); // Using the correct global context
    ...
    useEffect(() => {
        ...
        const progressListener = onValue(progressRef, (progressSnapshot) => {
            ...
            modulesListener = onValue(modulesRef, (modulesSnapshot) => {
                ...
                if (modules) { for (const moduleId in modules) { if (modules[moduleId].lessons && modules[moduleId].lessons[lessonId]) { foundLesson = modules[moduleId].lessons[lessonId]; break; } } }
                if (foundLesson) { setLessonData(foundLesson); } else { setError('Lesson data could not be found.'); }
                setLoading(false);
            });
        }, { onlyOnce: true });
        return () => { progressListener(); if (modulesListener) modulesListener(); };
    }, [user, lessonId]);
    
    // Correct useEffect for the Global Pop-up Bot
    useEffect(() => {
        if (lessonData?.chatbotId) {
            setBotId(lessonData.chatbotId); // Tell the global layout which bot to load
        }
        return () => {
            setBotId(null); // Clean up when we leave the page
        };
    }, [lessonData, setBotId]);
        ...
        <div className="lesson-page-wrapper">
            ...
            <main className="lesson-container">
                <div className="lesson-content">
                    ...
                    {/* The AI Mentor section is now just a simple prompt */}
